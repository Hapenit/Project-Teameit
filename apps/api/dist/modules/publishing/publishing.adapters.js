...
exports.publishMeta = publishMeta;
exports.publishLinkedIn = publishLinkedIn;
exports.publishPlatform = publishPlatform;
...
const json = async (response) => {
    ...
    if (!response.ok)
        throw new Error(`Provider request failed (${response.status}): ${body.error?.message || JSON.stringify(body)}`);
    return body;
};
async function credentials(tenantId, provider) {
    const value = await integrations_engine_1.IntegrationEngine.getCredentials(tenantId, provider);
...
async function publishMeta(platform, input) {
    const c = await credentials(input.tenantId, platform);
    const token = encodeURIComponent(c.access_token);
    if (platform === 'facebook') {
        const body = { message: input.content, access_token: c.access_token };
        if (input.media[0]?.media_url)
            body.link = input.media[0].media_url;
    ...
    if (!mediaUrl)
        throw new ProviderConfigurationError('Instagram publishing requires at least one public media URL');
    let mediaTypeParam = '';
    if (input.postType === 'story')
        mediaTypeParam = '&media_type=STORIES';
    else if (input.postType === 'reel')
        mediaTypeParam = '&media_type=REELS';
...
async function publishPlatform(platform, input) {
