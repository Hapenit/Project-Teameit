...
class LinkedInProvider {
    static authorizationUrl(redirectUri, state) {
    ...
    static async exchangeCode(code, redirectUri) {
        ...
        if (!response.ok || !data.access_token)
            throw new Error(`LinkedIn token exchange failed: ${data.error_description || response.statusText}`);
    ...
    static async publish(accessToken, authorUrn, text) {
        if (!authorUrn || !text.trim())
            throw new Error('LinkedIn author and content are required');
    ...
    static async request(path, token, init = {}) {
