...
class OAuthService {
     ...
     */
    static async generateAuthUrl(tenantId, provider) {
        if (!tenantId)
            throw new Error('Tenant context is required');
        if (!provider)
            throw new Error('Provider is required');
        if (!(0, provider_registry_1.isSupportedOAuthProvider)(provider)) {
        ...
        const { error: stateError } = await supabase_1.supabaseAdmin.from('oauth_states').insert({
            tenant_id: tenantId, provider, state_hash: stateHash,
            expires_at: new Date(Date.now() + 10 * 60 * 1000).toISOString()
        });
        if (stateError)
            throw new Error(`Unable to create OAuth state: ${stateError.message}`);
        ...
        switch (provider) {
            ...
            case 'whatsapp': {
                ...
                if (!metaAppId)
                    throw new Error('META_APP_ID is not configured. Please add it to your .env file.');
                const scopes = [
                    'email',
                    'pages_show_list',
                    'pages_messaging',
            ...
            case 'youtube': {
                ...
                if (!googleClientId)
                    throw new Error('GOOGLE_CLIENT_ID is not configured. Please add it to your .env file.');
                const scopeMap = {
            ...
            case 'linkedin': {
                ...
                if (!clientId)
                    throw new Error('LINKEDIN_CLIENT_ID is not configured. Please add it to your .env file.');
    ...
    static async handleCallback(code, state) {
        ...
        const stateHash = crypto.createHash('sha256').update(state).digest('hex');
        const { data: consumed, error: consumeError } = await supabase_1.supabaseAdmin.from('oauth_states')
        ...
        if (['meta', 'meta_ads', 'facebook', 'instagram', 'whatsapp'].includes(provider)) {
            ...
            if (!meData.id)
                throw new Error('Meta account lookup failed');
            ...
            if (!accountsRes.ok || accountsData.error)
                throw new Error(`Meta asset discovery failed: ${accountsData.error?.message || accountsRes.statusText}`);
            const assets = (accountsData.data || []).map((account) => ({
                id: account.id,
                name: account.name,
            ...
            for (const asset of assets) {
                if (!asset.whatsappBusinessAccountId || !asset.accessToken)
                    continue;
                ...
                if (!phonesRes.ok || phones.error)
                    throw new Error(`WhatsApp number discovery failed: ${phones.error?.message || phonesRes.statusText}`);
                asset.whatsappPhoneNumbers = (phones.data || []).map((phone) => ({ id: phone.id, displayPhoneNumber: phone.display_phone_number }));
            }
            ...
            if (!tokenRes.ok || !tokenData.access_token)
                throw new Error(`LinkedIn token exchange failed: ${tokenData.error_description || tokenRes.statusText}`);
            const meRes = await fetch('https://api.linkedin.com/v2/userinfo', { headers: { Authorization: `Bearer ${tokenData.access_token}` } });
            const me = await meRes.json();
            if (!meRes.ok || !me.sub)
                throw new Error('LinkedIn account lookup failed');
            accessToken = tokenData.access_token;
            expiresAt = new Date(Date.now() + (tokenData.expires_in || 3600) * 1000).toISOString();
        ...
        let { data: integration } = await supabase_1.supabaseAdmin
            .from('integrations')
            .select('id')
            ...
            .single();
        if (!integration) {
            const { data: newIntegration, error: newIntError } = await supabase_1.supabaseAdmin
                .from('integrations')
                .insert({ tenant_id: tenantId, provider, status: 'active' })
                .select('id')
                .single();
            if (newIntError)
                throw new Error(`Failed to create integration: ${newIntError.message}`);
        ...
        if (GOOGLE_PROVIDERS.includes(provider) && !refreshToken) {
            const { data: previous } = await supabase_1.supabaseAdmin.from('integration_credentials')
                .select('refresh_token').eq('integration_id', integration.id).maybeSingle();
            refreshToken = (0, credentials_1.decryptSecret)(previous?.refresh_token) || null;
        }
        await supabase_1.supabaseAdmin
            .from('integration_credentials')
            .delete()
            .eq('integration_id', integration.id);
        const { error: credError } = await supabase_1.supabaseAdmin
            .from('integration_credentials')
            .insert({
            integration_id: integration.id,
            access_token: (0, credentials_1.encryptSecret)(accessToken),
            refresh_token: (0, credentials_1.encryptSecret)(refreshToken),
        ...
        if (providerMetadata?.assets) {
            for (const asset of providerMetadata.assets) {
                ...
                for (const [assetProvider, assetId] of mappings) {
                    if (!assetId)
                        continue;
                    const { data: child } = await supabase_1.supabaseAdmin.from('integrations')
                        ...
                        .select('id').single();
                    if (!child)
                        throw new Error(`Failed to map Meta ${assetProvider} account`);
                    await supabase_1.supabaseAdmin.from('integration_credentials').delete().eq('integration_id', child.id);
                    const { error } = await supabase_1.supabaseAdmin.from('integration_credentials').insert({
                        integration_id: child.id,
                        access_token: (0, credentials_1.encryptSecret)(asset.accessToken || accessToken),
                        external_account_id: assetId,
                        metadata: { parentProvider: 'meta', assetName: asset.name }
                    });
                    if (error)
                        throw new Error(`Failed to store Meta ${assetProvider} credentials: ${error.message}`);
                }
                for (const phone of asset.whatsappPhoneNumbers || []) {
                    const { data: child } = await supabase_1.supabaseAdmin.from('integrations')
                        ...
                        .select('id').single();
                    if (!child)
                        throw new Error('Failed to map WhatsApp number');
                    await supabase_1.supabaseAdmin.from('integration_credentials').delete().eq('integration_id', child.id);
                    const { error } = await supabase_1.supabaseAdmin.from('integration_credentials').insert({
                        integration_id: child.id, access_token: (0, credentials_1.encryptSecret)(asset.accessToken || accessToken),
                    ...
                    if (error)
                        throw new Error(`Failed to store WhatsApp number credentials: ${error.message}`);
                }
            }
        }
     ...
     */
    static async disconnect(tenantId, provider) {
        ...
        const { data: integration } = await supabase_1.supabaseAdmin
            .from('integrations')
            .select('id')
            .eq('tenant_id', tenantId)
            .eq('provider', provider)
