...
export interface PublishingAdapter {
  publish(input: { content: string; media: Array<{ media_type: string; media_url: string }> }): Promise<string>;
}

export const SUPPORTED_PLATFORMS = ['facebook', 'instagram', 'linkedin'] as const;
...
export function validateCreatePost(params: CreatePostParams): void {
  ...
  const unsupported = platforms.filter(platform => !SUPPORTED_PLATFORMS.includes(platform as SupportedPlatform));
...
export class PublishingEngine {
  static async createPost(params: CreatePostParams) {
    ...
    if (mediaUrls.length > 0) {
      ...
      if (mediaError) throw new Error(`Failed to attach media: ${mediaError.message}`);
    }

    return post;
  }

  static async getPost(id: string, tenantId: string) {
    const { data, error } = await supabaseAdmin.from('posts').select('*, post_media(*)').eq('id', id).eq('tenant_id', tenantId).single();
    if (error || !data) throw new PublishingValidationError('Post not found');
    return data;
  }

  static async publishPost(tenantId: string, postId: string) {
