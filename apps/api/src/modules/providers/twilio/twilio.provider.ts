export class TwilioProvider {
  static async sendSms(to: string, body: string, config: { accountSid?: string; authToken?: string; from?: string; statusCallback?: string } = {}) {
    ...
    if (!to || !body.trim()) throw new Error('SMS recipient and body are required');
    ...
    if (!response.ok || !data.sid) throw new Error(`Twilio SMS failed: ${data.message || response.statusText}`);
